yOSON.AppCore.addModule("accordion_effect", function(Sb) {
  ...
  factory.prototype = {
    ...
    eShowHideWrap: function(event) {
      var dom, row, st, that, wrap, _this;
      ...
      if (wrap.is(':visible')) {
        wrap.slideUp('fast', function() {
          wrap.hide();
          row.text(st.contentOpen).addClass(st.classOpen).removeClass(st.classClose);
        });
      } else {
        if (st.oneAtATime) {
          _this.siblings(st.divAction).next().slideUp();
          _this.siblings(st.divAction).find(st.divIcon).text(st.contentOpen).addClass(st.classOpen).removeClass(st.classClose);
        }
        wrap.slideDown('fast', function() {
          wrap.show();
          row.text(st.contentClose).addClass(st.classClose).removeClass(st.classOpen);
        });
      }
    },
    execute: function() {
...
yOSON.AppCore.addModule("file_uploader", function(Sb) {
  ...
  factory = function(op) {
    this.st = {
      ...
      response: '.response',
      srcDefault: yOSON.statHost + '/images/empresa/photoEmpDefault.png',
  ...
  factory.prototype = {
    ...
    eDeleteFile: function(event) {
      ...
      if (st.isImage) {
        ...
        if (!dom.btnDelete.attr('data-exits')) {
          dom.filLogo.val('');
          that.fnSuccesDelete();
          return false;
        }
    ...
    fnSuccesDelete: function() {
      ...
      this.dom.btnDelete.removeAttr('data-exits');
    },
    execute: function() {
      this.st = $.extend({}, this.st, this.op);
      this.beforeCathDom();
...
yOSON.AppCore.addModule("validate_input_ajax", function(Sb) {
  ...
  factory.prototype = {
    ...
    suscribeEvents: function() {
      ...
      this.dom.context.on('focusout', this.st.txtInput, {
        inst: this
      }, this.eValidateInput);
    },
    ...
    eValidateInput: function(event) {
      ...
      utils.responseParsley('error', false, _this.siblings(st.divMessage));
      utils.responseParsley('success', false, _this.siblings(st.divMessage));
      if (!_this.hasClass(st.classError)) {
...
yOSON.AppCore.addModule("e_planning", function(Sb) {
  ...
  afterCatchDom = function() {
    st.eplArgs.sV = "" + (functions.schemeLocal()) + st.eplArgs.sV;
    functions.eplCheckStart();
    functions.setEplannings();
  };
  functions = {
    setEplannings: function() {
      dom.eplanningClass.each(function() {
        var _that, section;
        _that = $(this);
        section = _that.data('section');
        _that.html('<div id="eplAdDiv' + section + '"></div>');
        functions.eplAD4M(section, st.eplArgs.custom);
      });
    },
    eplCheckStart: function() {
      ...
      if (document.epl) {
        ...
        if (e.eplReady()) {
          ...
          e.eplInit(st.eplArgs);
          if (st.eplArgs.custom) {
        ...
        eS2 = void 0;
        dc = document.cookie;
        cookieName = ('https' === functions.schemeLocal() ? 'EPLSERVER_S' : 'EPLSERVER') + '=';
        ci = dc.indexOf(cookieName);
        if (ci !== -1) {
        ...
        eIFD = eIF.contentDocument ? eIF.contentDocument : eIF.document;
        eIFD.open();
        eIFD.write('<html><head><title>e-planning</title></head><body></body></html>');
        eIFD.close();
        s = eIFD.createElement('SCRIPT');
        s.src = functions.schemeLocal() + '://' + (eS2 ? eS2 : st.eS1) + '/layers/epl-41.js';
        eIFD.body.appendChild(s);
//        if (!eS2) {
//          ss = eIFD.createElement('SCRIPT');
//          ss.src = functions.schemeLocal() + '://ads.us.e-planning.net/egc/4/2912';
...
//        }
    ...
    eplSetAdM: function(eID, custF) {
      var stateCustF;
      if (functions.eplCheckStart()) {
        if (custF) {
          document.epl.setCustomAdShow(eID, st.eplArgs.custom[eID]);
        }
        document.epl.showSpace(eID);
      } else {
        stateCustF = custF ? true : false;
        setTimeout((function() {
    ...
    eplAD4M: function(eID, custF) {
      if (custF) {
        ...
        st.eplArgs.custom[eID] = custF;
      }
      functions.eplSetAdM(eID, custF ? true : false);
    },
