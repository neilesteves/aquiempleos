...
yOSON.AppCore.addModule("combos_depends", function(Sb) {
  ...
  factory.prototype = {
    ...
    eShowHideOptions: function(event) {
      ...
      selChild = $('#' + st.selChild, parentWrap);
...
yOSON.AppCore.addModule("count_character_for_skills", function(Sb) {
...
yOSON.AppCore.addModule("location_job", function(Sb) {
  ...
  //      dom.selDistrito.parsley().reset();
     ...
     // dom.selDistrito.attr('disabled', true);
      dom.selProvincia.find('option:first').siblings().remove();
      st.xhrGetList = $.ajax({
      url: st.urlGetToken,
      type: 'POST',
      dataType: 'JSON',
        ...
        st.xhrGetList = $.ajax({
          ...
          success: function(response) {
            $.each(response, function(i, data) {
              ...
              compiled_template = _.template(st.templateHtmlOption, objModel);
              dom.selProvincia.append(compiled_template);
            });
     ...
     // dom.selDistrito.attr('disabled', true).val(0);
               ///   console.log(value );
             //     console.log(st.dataDepartamentoLima );
...
yOSON.AppCore.addModule("search_filters", function(Sb) {
  ...
  events = {
    onSubmit: function(e) {
      var area, cadena, nivelpuestos, returnString, ubicaciones, value;
      e.preventDefault();
