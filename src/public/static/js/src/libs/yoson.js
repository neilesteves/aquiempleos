...
yOSON.AppScript = (function(statHost, filesVers){
    ...
    var execFncs = function(url){
        ...
        for(var i=0; i<ScrFnc[codear(url)].fncs.length; i++){
            ...
            console.log(ScrFnc[codear(url)].fncs[i]);
    ...
    var loadJs = function(url, fnc, onLoadScript){
        ...
        if(scr.readyState){  /*IE*/
            scr.onreadystatechange = function(){
              ...
              scr.onload=function(){
                  if(onLoadScript) {
                    //collecionJS.push(url)
                    //collecionFunc.push(onLoadScript)
                    onLoadScript();
                  } else {
                    fnc(url);
                  }
    ...
    return {
        chargeSync: function(aUrl, fFnc, sMod, lev){
            ...
            var loopArray = function(src){
                    ...
                    //var exists = collecionJS.indexOf(url);
                    /*if(exists !== -1){
                        //Ya está cargado
                        collecionJS[num]();
                        return false
                    ...
                    loadJs(srcItem, execFncs, function(){
                        ...
                        if(x < src.length){
                            ...
                            /*var exists = collecionJS.indexOf(srcItem)
                            if(exists !== -1){
                                console.log(exists);
                                //collecionFunc[exists]();
                            }else{
...
yOSON.AppCore = (function(){
    ...
    return {
        ...
        getModule: function(sModuleId){
            ...
            else{ console.log ( 'structureline58 param "sModuleId" is not defined or module not found' ); }
        },
        ...
        runModule: function(sModuleId, oParams){
            ...
            if(typeof oModules[sModuleId] !== "undefined"){
                ...
                }else{ console.log ( ' ---> init function is not defined in the module "'+oModules[sModuleId]+'"' ); }
...
yOSON.search = {
    ...
    tipoInmueble    : "ck_tipo_inmueble",
    ubicacion       : "ubicacion",
    precio          : "precio",
    habitaciones    : "rb_habitacion",
    bath            : "rb_bath",
    antiguedad      : "rb_antiguedad",
    cocheras        : "rb_cochera",
    fila            : "radFila",
    camas           : "radCama",
    publicacion     : "rb_publicacion",
    areaTotal       : "ck_area_total",
    areaConstruida  : "ck_area_construida",
    servicios       : "ck_service_adittional",
    estadia         : "chkEstadia",
    empresasAgentes : "empresasAgentes",
    distancia       : "distancia",
    fecha           : "fecha",
    playas          : "playas",
    keyword         : "keyword",
    projectState    : "radProjectState",
    lat             : "lat",
    lng             : "lng",
    core            : "core"
};
...
yOSON.search.fields = {
    precio : {
        moneda      : "rb_tipo_moneda",
        opcion1     : "rb_precio",
        opcion2     : "opcion2"
    },
    distancia : {
        orientacion : "radTipoDistancia",
        opcion1     : "radDistancia"
    },
    ubicacion       : ['dep','pro','dis','urb'],
    playas          : ['dep','pro','dis','playa'],
    fecha           : {
        opcion1 : 'opcion1'
    }
};

//codigo para retrasar la carga de los elementos en el home

$(document).ready(function() {
    //home
    $('#featuredProjects', $('#wrapper')).children('.urb-ads').each( function(i,div){
        var $div = $(div);
        var imgsCollection = $div.find('img');
        imgsCollection.each(function(i,e){
            var $e = $(e);
            srcTemp = $e.data('src');
    ...
    $('.lt-ads','#divPropertyCompaniesAndAgents').each(function(i,div){
        var $div = $(div);
        var imgsCollection = $div.find('img');
        imgsCollection.each(function(i,e){
            var $e = $(e);
    ...
    $('.urb-ads','#addHighlights').each(function(i,div){
        var $div = $(div);
        var imgsCollection = $div.find('img');
        imgsCollection.each(function(i,e){
            var $e = $(e);
    ...
    $('img',$('#divBanner2')).each(function(i,img){
    ...
    $('img', $('.urb-ads .content-proy a')).each(function(i,e){
        var $e = $(e);
        srcTemp = $e.data('src') || '';
        log(srcTemp);
        if (srcTemp != '') {
    ...
    //Elige tu tipo de departamento
    $('img', $('.img-kdepart')).each(function(i,e){
        var $e = $(e);
        srcTemp = $e.data('src') || '';
        if (srcTemp != '') {
            $e.removeAttr('data-src');
