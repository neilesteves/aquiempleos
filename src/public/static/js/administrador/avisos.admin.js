...
var Administradores = (function(){
   var  version,
        ...
        assignProcess = function(){
            ...
            formAdmin.bind('submit',function(event){
                ...
                var estado,
                    ...
                    if(administradorId != "" && administradorId != undefined){
                        $.ajax({
                            url : formAdminAction+'/administrador_id/'+administradorId,
                            beforeSend : function(){
                                flagClose=false;
                                $('#loading-assigProcess').removeClass('none');
                                $('#btnFormAdmin').attr('disabled','disabled');
                                 $('.abtnDelete').unbind('click').attr('href','javascript:;');
                            },
                            success : function(data){
                                flagClose=true;
                                estado = data.estado;
                                ...
                                MsgBox.showBox(data.mensaje,'msgResult'+estado,5000);
                                    showAndRemoveManager();


                            } 
                        });
                    }else{
                        MsgBox.showBox("Debe seleccionar un Administrador",'msgResultInter',5000); 
                    }
        ...
        deleteAdmin =  function(){
            var urlUpdate;
            $(".abtnDelete").bind('click',function(event){
                urlUpdate = $(this).attr('href');
                event.preventDefault();
                $.ajax({
                    url : urlUpdate,
                    beforeSend : function(){
                        flagClose=false;
                        $("#loading-delete").removeClass('none').css({'marginLeft':'10px'});
                        $('#btnFormAdmin').attr('disabled','disabled');
                        $('.abtnDelete').unbind('click').attr('href','javascript:;');
                    },
                    success : function(data){
        ...
        showAndRemoveManager = function(){
            $('#dataResutlAdmin').slideUp('slow',function(){
                $(this).remove();
                showLoadingAdmin(true);
                $.ajax({
                    url : $('#managerEdit').attr('data-anuncion-id'),
                    success : function(data){
                        showLoadingAdmin(false,function(){
                            dom.managerEdit.hide().append(data).slideDown('slow');
                            deleteAdmin();
