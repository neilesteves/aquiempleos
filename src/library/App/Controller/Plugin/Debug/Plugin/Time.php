 ...
 * @license    http://code.google.com/p/zfdebug/wiki/License     New BSD License
 */
class App_Controller_Plugin_Debug_Plugin_Time extends Zend_Controller_Plugin_Abstract
    implements App_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getLogger()
    {
        if (!$this->_logger) {
            $this->_logger = Zend_Controller_Front::getInstance()
                    ->getPlugin('App_Controller_Plugin_Debug')->getPlugin('Log')->getLog();
     ...
     *
     * @param string $name
     * @deprecated Use App_Controller_Plugin_Debug_Plugin_Log
     */
    public function mark($name)
    {
        $this->getLogger()->mark("$name");
        trigger_error("App Time plugin is deprecated, use the Log plugin");
    }
    ...
    public function getDispatchStatistics()
    {
        ...
        $timerNamespace->data[$this_module][$this_controller][$this_action][] = round($this->_timer['dispatchLoopShutdown'],
            2);
