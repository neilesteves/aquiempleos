 ...
 * @license    http://code.google.com/p/zfdebug/wiki/License     New BSD License
 */
class App_Controller_Plugin_Debug_Plugin_Exception extends Zend_Controller_Plugin_Abstract
    implements App_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    {
        if (!self::$_logger) {
            if ($zfdebug = Zend_Controller_Front::getInstance()->getPlugin('App_Controller_Plugin_Debug')) {
    ...
    public static function errorHandler($level, $message, $file, $line)
    {
        ...
        $message = sprintf(
            "%s in %s on line %d", $message,
            str_replace($_SERVER['DOCUMENT_ROOT'], '', $file), $line
        );
    ...
    public function dispatchLoopShutdown()
    {
        ...
        foreach ($response->getException() as $e) {
            $exception = get_class($e) . ': ' . $e->getMessage()
                . ' thrown in ' . str_replace($_SERVER['DOCUMENT_ROOT'], '',
                    $e->getFile())
                . ' on line ' . $e->getLine();
            $exception .= '<ol>';
            foreach ($e->getTrace() as $t) {
                ...
                if (!isset($t['line'])) $t['line'] = 'n/a';
                $exception .= '<li>' . $func . ' in '
                    . str_replace($_SERVER['DOCUMENT_ROOT'], '', $t['file'])
                    . ' on line ' . $t['line'] . '</li>';
            }
            $exception .= '</ol>';
            if ($logger = self::getLogger()) $logger->crit($exception);
        }
    }
