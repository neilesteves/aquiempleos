...
class App_View_Helper_SelectAttribs extends Zend_View_Helper_FormElement {
    public function selectAttribs($name, $value = null, $attribs = null, $options = null, $listsep = "<br />\n") {
        ...
        foreach ($options as $opt_value => $option) {
            ...
            $list[] = $this->_build($value, $option, $disabled);
        }
    ...
    protected function _build($setvalue, $option, $disabled) {
        
        $html = '<option';
        
        if(is_array($setvalue) and in_array($option['value'], $setvalue))             
