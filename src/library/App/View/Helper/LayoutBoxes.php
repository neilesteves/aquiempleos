...
class App_View_Helper_LayoutBoxes extends Zend_View_Helper_Abstract
{

    public function LayoutBoxes($layout)
    {
        $box  = '';
        ...
        if (!isset($auth['usuario'])) {
            if (isset($layout->loginFormNew) && $layout->loginFormNew) {
                $box .= $this->view->partial('boxes/all/_new_box_login.phtml',
                    array(
            ...
            if (MODULE == "postulante") {
                if (isset($layout->formRegistroRapido) && $layout->formRegistroRapido) {
                    $box .= $this->view->partial('boxes/all/_new_box_register.phtml',
                        array(
...
//          $box .= $this->view->partial('boxes/postulante/_new_box_information_modal.phtml',array(
...
//          ));  
        ...
        if (isset($layout->newrecuperarClaveForm) && $layout->newrecuperarClaveForm) {
            $box .= $this->view->partial('boxes/postulante/_new_box_recuperaclave.phtml',
                ...
                'return' => $layout->params['REQUEST_URI']
            ));
        }
